...
// Метод для записи списка в бинарный файл
template <typename T>
void DoublyLinkedList<T>::saveToBinaryFile(const string& filename) const {
    ofstream outFile(filename, ios::binary);
    if (!outFile) {
        throw runtime_error("Не удалось открыть файл для записи.");
    }
    
    outFile.write(reinterpret_cast<const char*>(&size), sizeof(size)); // Записываем размер списка
    
    Node* curr = head;
    while (curr) {
        outFile.write(reinterpret_cast<const char*>(&curr->data), sizeof(curr->data)); // Записываем значение элемента
        curr = curr->next; // Переходим к следующему элементу
    }
...
// Метод для чтения списка из бинарного файла
template <typename T>
void DoublyLinkedList<T>::loadFromBinaryFile(const string& filename) {
    ...
    int newSize;
    inFile.read(reinterpret_cast<char*>(&newSize), sizeof(newSize)); // Читаем размер списка
    ...
    for (int i = 0; i < newSize; ++i) {
        inFile.read(reinterpret_cast<char*>(&value), sizeof(value)); // Читаем значение элемента
        LpushBack2(value); // Добавляем элемент в конец списка
    }
