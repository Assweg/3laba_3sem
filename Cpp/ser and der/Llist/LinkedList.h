...
template <typename T>
void LinkedList<T>::saveToBinaryFile(const string& filename) const {
    ...
    Node* curr = head;
    while (curr) {
        outFile.write(reinterpret_cast<const char*>(&curr->data), sizeof(curr->data));
        curr = curr->next;
    }
...
template <typename T>
void LinkedList<T>::loadFromBinaryFile(const string& filename) {
