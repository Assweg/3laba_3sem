...
// Метод для записи стека в бинарный файл
template <typename T>
void Stack<T>::saveToBinaryFile(const string& filename) const {
    ofstream outFile(filename, ios::binary);
    if (!outFile) {
        throw runtime_error("Не удалось открыть файл для записи.");
    }

    // Записываем индекс верхнего элемента
    outFile.write(reinterpret_cast<const char*>(&top), sizeof(top));

    // Записываем элементы стека
    for (int i = 0; i <= top; ++i) {
...
// Метод для чтения стека из бинарного файла
template <typename T>
void Stack<T>::loadFromBinaryFile(const string& filename) {
    ...
    // Читаем индекс верхнего элемента
