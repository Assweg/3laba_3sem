...
// Метод для записи дерева в бинарный файл
template <typename T>
void CompleteBinaryTree<T>::saveToBinaryFile(const string& filename) const {
    ofstream outFile(filename, ios::binary);
    if (!outFile) {
        throw runtime_error("Не удалось открыть файл для записи.");
    }

    // Записываем текущий размер дерева
    outFile.write(reinterpret_cast<const char*>(&size), sizeof(size));

    // Записываем элементы дерева
    for (int i = 0; i < size; ++i) {
        outFile.write(reinterpret_cast<const char*>(&tree[i]), sizeof(tree[i]));
    }

    outFile.close();
}

// Метод для чтения дерева из бинарного файла
template <typename T>
void CompleteBinaryTree<T>::loadFromBinaryFile(const string& filename) {
    ifstream inFile(filename, ios::binary);
    if (!inFile) {
        throw runtime_error("Не удалось открыть файл для чтения.");
    }

    // Сбрасываем текущее дерево
    clear();

    // Читаем текущий размер дерева
